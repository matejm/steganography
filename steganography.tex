\documentclass[a4paper, 12pt]{article}
\usepackage[slovene]{babel}
\usepackage[utf8]{inputenc}
\usepackage{minted}
\usepackage{nameref}
\usepackage[backend=bibtex]{biblatex}
\usepackage{graphicx}

\graphicspath{{images/}}

\title{
    Programiranje v Pythonu\\\large
    Izdelava programa za steganografijo slik}
\author{Matej Marinko}
\date{februar 2017}


\addbibresource{latex/viri.bib}

\begin{document}
    \maketitle
    \thispagestyle{empty}
    \newpage

    % \begin{abstract}
    % \end{abstract}
    % \newpage

    \tableofcontents
    \newpage

    \section{Uvod} 
        \input{latex/uvod.tex}

    \newpage

    \section{Steganografija}
        \input{latex/steganografija.tex}

          
    \section{Zapis slik v računalniku}
        \input{latex/zapis_slik.tex}


    \section{AES kriptiranje}
        \input{latex/aes.tex}

    \section{Python}
        \input{latex/python.tex}

    \section{Izdelava programa}
        \input{latex/programiranje.tex}        

    \newpage
    \section{Zaključek}
        \input{latex/zakljucek.tex}

    \newpage
    \printbibliography
     
\end{document}

